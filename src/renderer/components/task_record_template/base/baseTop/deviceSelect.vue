      ...
      <div>
        ...
        <div v-if="data">
          ...
          <el-radio-group v-model="tensioningPattern" size="medium" >
            <el-radio-button :label="item" v-for="(item, index) in tensioningPatternSort" :key="index">{{tensioningPatternStr[item]}}--{{item}}</el-radio-button>
          </el-radio-group>
        </div>
      </div>
...
export default {
  ...
  computed: {
    tensioningPatternSort() {
      return this.data.tensioningPattern.sort();
    },
  },
  beforeMount() {
